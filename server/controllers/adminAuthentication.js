...
exports.signup = function(req, res, next) {
    ...
    AdminUser.findOne({email: email}, function(err, existingUser) {
        if(err) {
            return next(err);
        }
...
exports.changePassword = function(req, res, next) {
    ...
    AdminUser.findById(req.user._id)
        .then((user) => {
...
exports.editInfo = function(req, res, next) {
    ...
    AdminUser.findByIdAndUpdate(req.user._id, {fullname, address, city, country}, {new: true})
        .then((user) => {
            res.send({info: {
